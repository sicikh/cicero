...
async fn compile_scenarios(path: impl AsRef<std::path::Path>) -> HashMap<u64, Scenario> {
    ...
    while let Some(entry) = dirs.next_entry().await.unwrap() {
        let path = entry.path();
        if path.is_dir() {
            let scenario = tokio::task::spawn_blocking(move || compile_scenario(path)).await.unwrap().unwrap();
...
async fn server_fn_handler(State(app_state): State<AppState>, request: Request<AxumBody>) -> impl IntoResponse {
